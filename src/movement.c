...
static void md_inet6_iface_init(struct md_inet6_iface *i, int ifindex)
{
	...
	/* Copy useful configuration information for interface:
	 * tunnel status, link up delay, ...  */
	list_for_each(list, &conf.net_ifaces) {
		...
		if (nif->ifindex == ifindex) {
			...
			tscpy(i->link_up_delay, nif->link_up_delay);
		}
	}
...
static void md_link_up(struct md_inet6_iface *iface)
{
	...
	dbg("Waiting %d ms before sending RS for iface %s\n",
	    tstomsec(iface->link_up_delay), iface->name);

	/* For some interfaces, we need to wait a bit after link up
	 * event before sending first RS or do tunnel work */
	if (tsisset(iface->link_up_delay)) {
		struct timespec req, rem;
		tscpy(req, iface->link_up_delay);
...
int md_start(void)
{
	 ...
	 * After a the expected dealy, md_movements_disabled is set
